...
def build_dimensions(title_akas, title_basics):
    ...
    # Build dim_time (based on startyear)
...
def build_fact_and_bridge(title_akas, title_basics, title_ratings, dim_region, dim_language, dim_genre, dim_time):
    print("Building fact and bridge tables ...")

    merged = title_basics.merge(title_ratings, on="tconst", how="left")
    ...
    fact_film_version["average_rating"] = fact_film_version["average_rating"].fillna(0)
    fact_film_version["num_votes"] = fact_film_version["num_votes"].fillna(0).astype(int)
    ...
    fact_genre_bridge["genre_id"] = fact_genre_bridge["genres"].map(genre_map).fillna(-1).astype(int)
